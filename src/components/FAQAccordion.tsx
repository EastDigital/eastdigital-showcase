...
const FAQAccordion: React.FC<FAQAccordionProps> = ({
  ...
  return <section className={`py-10 ${className} relative overflow-hidden`}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-primary/8 rounded-full blur-3xl opacity-60" 
             style={{animation: 'gentle-float 8s ease-in-out infinite'}}></div>
        <div className="absolute top-3/4 right-1/4 w-24 h-24 bg-accent/6 rounded-full blur-2xl opacity-50" 
             style={{animation: 'gentle-float 12s ease-in-out infinite reverse', animationDelay: '2s'}}></div>
        <div className="absolute bottom-1/3 left-1/2 w-40 h-40 bg-primary/4 rounded-full blur-3xl opacity-40"
             style={{animation: 'gentle-float 15s ease-in-out infinite', animationDelay: '4s'}}></div>
        
        {/* Minimal geometric accents */}
        <div className="absolute top-20 right-20 w-16 h-16 border border-primary/10 rounded-lg opacity-30"
             style={{animation: 'subtle-rotate 20s linear infinite'}}></div>
        <div className="absolute bottom-20 left-20 w-12 h-12 border border-accent/8 rounded-full opacity-25"
             style={{animation: 'gentle-pulse 6s ease-in-out infinite'}}></div>
